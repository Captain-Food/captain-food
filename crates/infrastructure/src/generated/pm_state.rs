...
const PAYMENT_PROCESS_COLUMNS: &str = "cart_id, order_id, payment_intent_id, process_status, payment_status, customer_id, session_id, client_secret, last_processed_stripe_event_id, last_update_utc";

fn decode_payment_process(row: &PgRow) -> Result<PaymentProcessRow, DomainError> {
    Ok(PaymentProcessRow {
        ...
        last_processed_stripe_event_id: row.try_get::<Option<String>, _>("last_processed_stripe_event_id").map_err(db_err)?.map(ExternalReference),
...
impl PaymentProcessStateStore for PgPaymentProcessState {
    async fn by_cart(&self, cart_id: CartId) -> Result<Option<PaymentProcessRow>, DomainError> {
        let sql = format!("SELECT {PAYMENT_PROCESS_COLUMNS} FROM payment_process_manager WHERE cart_id = $1");
        let row = sqlx::query(&sql).bind(cart_id.0).fetch_optional(&self.pool).await.map_err(db_err)?;
        row.as_ref().map(decode_payment_process).transpose()
    }

    async fn by_payment_intent(&self, payment_intent_id: &PaymentIntentId) -> Result<Option<PaymentProcessRow>, DomainError> {
        let sql = format!("SELECT {PAYMENT_PROCESS_COLUMNS} FROM payment_process_manager WHERE payment_intent_id = $1");
        let row = sqlx::query(&sql).bind(payment_intent_id.0.clone()).fetch_optional(&self.pool).await.map_err(db_err)?;
        row.as_ref().map(decode_payment_process).transpose()
    }

    async fn by_order(&self, order_id: OrderId) -> Result<Option<PaymentProcessRow>, DomainError> {
        let sql = format!("SELECT {PAYMENT_PROCESS_COLUMNS} FROM payment_process_manager WHERE order_id = $1");
        let row = sqlx::query(&sql).bind(order_id.0).fetch_optional(&self.pool).await.map_err(db_err)?;
        row.as_ref().map(decode_payment_process).transpose()
    }
...
const REFUND_PROCESS_COLUMNS: &str = "order_id, payment_intent_id, refund_id, process_status, approved_amount_cents, reason, last_update_utc";
...
impl RefundProcessStateStore for PgRefundProcessState {
    async fn by_order(&self, order_id: OrderId) -> Result<Option<RefundProcessRow>, DomainError> {
        let sql = format!("SELECT {REFUND_PROCESS_COLUMNS} FROM refund_process_manager WHERE order_id = $1");
        let row = sqlx::query(&sql).bind(order_id.0).fetch_optional(&self.pool).await.map_err(db_err)?;
        row.as_ref().map(decode_refund_process).transpose()
    }
...
impl CartBindingStateStore for PgCartBindingState {
    async fn by_session(&self, session_id: SessionId) -> Result<Option<CartBindingRow>, DomainError> {
        let sql = format!("SELECT {CART_BINDING_COLUMNS} FROM cart_binding_process_manager WHERE session_id = $1");
        let row = sqlx::query(&sql).bind(session_id.0).fetch_optional(&self.pool).await.map_err(db_err)?;
...
impl DeliveryDispatchStateStore for PgDeliveryDispatchState {
    async fn by_order(&self, order_id: OrderId) -> Result<Option<DeliveryDispatchRow>, DomainError> {
        let sql = format!("SELECT {DELIVERY_DISPATCH_COLUMNS} FROM delivery_dispatch_process_manager WHERE order_id = $1");
        let row = sqlx::query(&sql).bind(order_id.0).fetch_optional(&self.pool).await.map_err(db_err)?;
        row.as_ref().map(decode_delivery_dispatch).transpose()
    }

    async fn by_delivery_job(&self, delivery_job_id: DeliveryJobId) -> Result<Option<DeliveryDispatchRow>, DomainError> {
        let sql = format!("SELECT {DELIVERY_DISPATCH_COLUMNS} FROM delivery_dispatch_process_manager WHERE delivery_job_id = $1");
        let row = sqlx::query(&sql).bind(delivery_job_id.0).fetch_optional(&self.pool).await.map_err(db_err)?;
        row.as_ref().map(decode_delivery_dispatch).transpose()
    }
