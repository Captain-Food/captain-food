...
Each critical workflow declares: `workflow` ($ref bindings to saga/command/events), `run_identity`
(must include `correlation_id` + `trace_id`), `spans` (each with an OTel `kind` in
