...
| [20260722-091500](20260722-091500-sdui-screens-audience-taxonomy.md) | **SDUI screens organized by audience; drop the `_screens` suffix** (refines ADR-0037 §2/§4) — one `specs/screens/*.yaml` per audience, **no `_screens` suffix** (folder conveys it): `restaurant_frontoffice.yaml` (renamed from `customer_screens.yaml` — the customer storefront at `{slug}.captain.food`, PUBLIC+CUSTOMER), then `restaurant_backoffice.yaml`/`rider.yaml`/`system.yaml`. Audience sets **may** include a `system` set (lifts §4's blanket "no admin screen set"; impersonation stays for "view as"). Validator already generic over `screens/*.yaml`; only the doc/registry emitters were pinned to the old name. No API/behaviour change |
