...
- [specs/database/](specs/database/) — the store schema as DSL (ADR-0037): `tables/*.yaml` (real tables, globbed — `tables/eventstore.yaml` = `domain_events` + `domain_stream`; `tables/referential.yaml` = seed/config `View_*` tables configured once by a repo seed script, not projected), `projection_views.yaml` (the event-fed `View_*` read models — SQL VIEWS / materialized projections over `domain_events`, each declared by its source aggregate/events + filters + columns), and `functions/*.sql` (event-store functions). Generated to `specs/generated/schema.generated.sql` (+ one `ref_<enum>` lookup table per scalar enum). [specs/database.md](specs/database.md) is the narrative rationale (event-store concept, retention); query→view mapping is the `@reads` binding in [specs/api.yaml](specs/api.yaml).
...
- [specs/screens/](specs/screens/) — **Spec-Driven SDUI** apps (ADR-0033/0037), one file per audience: `customer_screens.yaml` today (restaurant/rider to follow). Each: screens + component registry + a **`resolvers`** allowlist (reads → `api.yaml` queries by `$ref`) + an **`actions`** allowlist (writes → `api.yaml` mutations by `$ref`). Each screen declares `roles` (⊆ UserType — so PUBLIC+CUSTOMER share one app) and the file declares `app_types` (web/ios/android/windows); admin uses impersonation, not its own screens. The validator (generic over `screens/*.yaml`) proves the **API answers the UI**; UI needs the API lacks are explicit `gaps`; `sdui: false` marks non-SDUI screens. Runtime (renderer/registry/Supabase) is a deferred contract.
